...
public static String dubLetterHelper(String input)
{
	String doubleLetter = "";
	for (int i = 0; i < input.length(); i++)
	{
		doubleLetter = doubleLetter + input.charAt(i) + input.charAt(i);
	}
	return doubleLetter;
}

public static void dubLetter(ArrayList<String> inputs)
{
	ArrayList<String> dubLetter = new ArrayList<String>();
	for (int i = 0; i < inputs.size(); i++)
	{
		dubLetter.add(dubLetterHelper(inputs.get(i)));
	}
	System.out.println(dubLetter);
}
...
public static boolean isPalindrome(String wordInput)
{
	...
	{
		revWord = revWord + wordInput.charAt(i);
	}
	if (revWord.equals(wordInput))
	{
		isPalindrome = true;
	}
	return isPalindrome;
...
public static void allPalindromes(ArrayList<String> palindromes)
{
	for (int i = 0; i < palindromes.size(); i++)
	{
		if (isPalindrome(palindromes.get(i)) == false)
		{
			palindromes.remove(palindromes.get(i));
		}
	}
